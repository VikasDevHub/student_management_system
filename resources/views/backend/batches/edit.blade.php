                    ...
                    <div class="card-body">
                        ...
                        <div class="row my-4">
                            ...
                            <div class="col-12 mb-3">
                                ...
                                <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $batch->name) }}" placeholder="Enter Name">
                                @error('name')
                            ...
                            <div class="col-12 mb-3">
                                ...
                                <input type="number" class="form-control" id="size_of_batch" name="size_of_batch" value="{{ old('size_of_batch', $batch->size_of_batch) }}" placeholder="Enter Size">
                                @error('size_of_batch')
                                <div class="text-danger text-sm my-1">{{ $message }}</div>
                                @enderror
                            </div>
                                ...
                                <select class="form-select" name="batch_admin">
                                    ...
                                    @if(!empty($teachers))
                                        @foreach($teachers as $teacher)
                                            <option value="{{ $teacher}}" {{ $batch->batch_admin == $teacher ? 'selected' : '' }}>{{ $teacher}}</option>
                                        @endforeach
                                    @endif
                                </select>
                                @error('batch_admin')
